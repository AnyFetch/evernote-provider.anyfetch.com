...
module.exports = function formatAndUploadNote(task, anyfetchClient, accessToken, cb) {
  var client = new Evernote.Client({
    token: accessToken, sandbox: config.clientConfig.sandbox
  });
  var NoteStore = client.getNoteStore();
  async.waterfall([
    function getNote(cb) {
      NoteStore.getNote(accessToken, task.identifier, true, true, true, true, cb);
    },
    ...
    function uploadNote(note, keywords, cb) {
      ...
      var newNote = {
        ...
        modification_date: task.updated,
